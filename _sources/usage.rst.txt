...
.. list-table::
   ...
   * - Import students from a roster CSV
     - ``course --add-csv roster.csv``
   * - Onboard a registration form to both platforms
     - ``course --onboard``
...
A few adapter constants mirror rules enforced by the extension itself -- the assignment
slug shape, the flags refused alongside ``--empty-repo``, and the roster CSV header -- so
this surface is coupled to the installed version. It was verified against ``gh teacher``
v1.45.0, which is pinned rather than tracking latest. ``--classroom50-preflight`` checks
only that the extension exists, not which version it is. The couplings that can drift on an
upgrade are listed under "Version coupling with ``gh teacher``" in the README.
...
A host with no browser needs nothing extra: ``authorize`` finds no browser to open,
starts no loopback listener, and prints the consent URL to open on a machine that has
one. That browser then fails to load ``http://127.0.0.1:8911/``, which is expected;
its address bar holds the redirect. Paste that URL back at the hidden prompt in the
same terminal and the command exchanges it for the token. ``--no-open-browser`` takes
the same path on a host that does have a browser.

The paste is checked for the exact ``http://127.0.0.1:8911/`` boundary and one
nonempty OAuth code and state before it is used, a redirect from a different run is
refused by the state it carries, and a malformed paste is asked for again rather than
ending the run. It stays out of shell history and process arguments because the prompt
hides it. Never reuse an earlier redirect URL; its code is single-use.

``complete-loopback --port PORT`` remains for the other case: an authorization that is
already listening here, started with a browser that then could not reach this host.
Read the port from the redirect URL and deliver it from a second terminal:
...
.. code-block:: bash
   ...
   ~/.course_venv/bin/python scripts/test_gclass_invite.py
   ~/.course_venv/bin/python scripts/test_c50_admin_cli.py
   ~/.course_venv/bin/python scripts/test_course_agents.py
   ~/.course_venv/bin/python scripts/test_classroom50.py
   ~/.course_venv/bin/python scripts/test_roster_csv.py
   ~/.course_venv/bin/python scripts/test_cli_flags.py
   ~/.course_venv/bin/python -m compileall -q course_hoanganhduc

The current offline run passes 259 tests across those eight ``unittest`` scripts, all 223
CLI flag parse cases, and the package compilation check. ``test_roster_csv.py`` and
``test_gclass_invite.py`` need no optional dependency and also run under a bare
``python3``.
...
teacher of the course, or already holding a pending invitation is skipped, so re-running the
command is a no-op:
...
- Candidates come from the local database only. ``--gc-invite-email`` filters that database; an
  address that is not in it is reported as ``skipped_not_in_db`` rather than invited, so import it
  first with ``--add-google-sheet`` or ``--add-csv``.
...
- Successful invitations record ``Google_ID``, ``Google_Invitation_ID``, and
  ...
  and is never overwritten; the recorded value is what makes the second run cost no
  ``userProfiles.get`` calls. ``Google_Invitation_ID`` and ``Google_Classroom_Invited_At``
  describe the latest invitation, so each successful invitation replaces them.
- A ``403`` is a course-wide policy failure rather than a per-student one, so the run stops there
  and the remaining candidates are reported as ``skipped_aborted``.
- A ``400 FAILED_PRECONDITION`` means the student already holds this role or greater, or the
  account is disabled; it is reported as ``skipped_precondition`` and is not a failure.
- The Classroom50 half is a separate command; see :doc:`cli_reference` under Classroom50.

Sheet to both platforms, end to end:

.. code-block:: bash

   course --add-google-sheet "<SHEET_URL>"                            # 1. Sheet -> database
   course --invite-google-classroom --google-course-id <COURSE_ID>    # 2. database -> Classroom
   course-c50-admin roster-import --org <ORG> --classroom <SHORT> --db students.db

Step 1 also accepts a plain CSV, which needs no Google account:

.. code-block:: bash

   course --add-csv roster.csv                                        # 1. CSV -> database

Step 3 is a separate command on purpose: it refuses in agent mode and without a terminal, and it
carries its own confirmation naming both of its effects. Each half is idempotent, so re-running
the one that failed is the whole recovery procedure. ``roster-import`` reads the organization
membership before deciding that a matching roster means there is nothing to do, so a run that
died after the roster commit but before the invitations still re-sends them; ``--force`` imports
a matching roster outright when that check cannot run.
...
- ``--dry-run`` reads and merges the sheet exactly as a real import would, prints every
  field a newer submission would replace, and writes nothing.
- A row whose student number is not 8 digits is kept, not dropped: the value is stored as
  ``Invalid Student ID``, ``Student ID`` is left empty, and the student is named. A row
  that names nobody is dropped, also by name.

Import students from a roster CSV with fixed columns:
...
- ``Student ID`` and ``Name`` are required, and so is at least one of ``Email`` and
  ``GitHub Username``; without one of those the roster feeds neither platform. ``GitHub ID``,
  ``Section``, and ``Class`` are read when present and every other column is ignored.
- Header names are matched case- and accent-insensitively, so ``Mã sinh viên``, ``Họ và tên``,
  ``Email``, ``GitHub username`` and ``Lớp`` all work. A header that matches nothing is refused,
  and the message lists the columns that were actually read.
- Rows with no student id or no name are skipped and counted; they do not fail the run.
- The file is decoded as UTF-8 (BOM or not), then Windows-1252, then Latin-1, and the fallback
  encoding is named when one is used. ``course-c50-admin roster-import --csv`` makes the opposite
  choice and refuses anything but UTF-8, because a mis-decoded name would be pushed to GitHub.
- ``--dry-run`` previews the first rows and writes nothing.
- This does not replace ``--add-file``, which still handles Excel, PDF, and the ``MAT*.xlsx``
  files with their own header quirks.
