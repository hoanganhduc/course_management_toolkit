...
- [Classroom50 (foundation50)](#classroom50-foundation50)
  - [Version coupling with `gh teacher`](#version-coupling-with-gh-teacher)
...
| Import students from a roster CSV | `course --add-csv roster.csv` |
| Onboard a registration form to both platforms | `course --onboard` |
...
- Classroom50 **teacher** extension available (`gh teacher …`), at a version this toolkit
  has been checked against — see [Version coupling with `gh teacher`](#version-coupling-with-gh-teacher)
- Local `students.db` (or path you pass) for sync/export

### Version coupling with `gh teacher`

The adapter never passes free-form arguments; it builds fixed `gh teacher` command lines and
validates the operands first. A few constants therefore mirror rules that actually live in the
extension, and they can drift when the extension is upgraded. Everything below was verified
against **v1.45.0** (commit `a5a51293b65d`), upgraded from v1.25.1 on 2026-09-05.

| Coupled to the CLI | Where | Note |
| --- | --- | --- |
| Assignment slug shape | `c50_cli_human.py` `_SLUG_RE` | Capped at 38 characters. v1.33.0 raised the CLI's own cap to 99 but also added a per-repo name budget this regex does not model, so the stricter cap is kept deliberately |
| Flags refused with `--empty-repo` | `c50_cli_human.py` `assignment_add_argv` | v1.45.0 refuses two more than v1.25.1 (`--submission-mode`, `--submission-tag`); the builder cannot emit either, so the list stays complete |
| `--feedback-pr` defaults to true | `c50_cli_human.py` `assignment_add_argv` | Disabling needs `=false`; unchanged across both versions |
| Roster CSV header | `c50_roster.py` `CANONICAL_COLUMNS` | Six columns, the canonical seven minus `role`; accepted by `ParseImportCSV` on both versions, and pinned by a test |
| `roster list --json` shape | `c50_roster.py` `parse_roster_payload` | The seventh field, `role`, is dropped on purpose |
| `roster import` is an upsert | `c50_admin_cli.py` `roster-import` | Rows absent from the CSV are left untouched, and every listed non-member is invited once the commit lands |
| CSV encoding | `c50_admin_cli.py` `_csv_rows_from_file` | Read as strict UTF-8, so a Windows-1252 file is refused here with `value_error` and exit 2 before `gh` runs. v1.34.0 added a Windows-1252 fallback to the extension, but that only helps when `gh teacher roster import` is typed directly; the `--db` path never produces such a file. `course --add-csv` deliberately chooses the opposite trade-off — see below |

`course --classroom50-preflight` checks only that `gh teacher` **exists**; it does not read the
version. Check it yourself:

```bash
gh teacher --version
gh extension list | grep teacher
```

The extension is pinned rather than tracking latest, so `gh extension upgrade` will refuse it.
Changing the pin means `gh extension remove teacher && gh extension install foundation50/gh-teacher --pin vX.Y.Z`.
After any such change, re-read the table above before trusting the adapter.
...
# Write the roster from the local database, and invite the students into the org
course-c50-admin roster-import --org my-org --classroom short-name --db students.db
course-c50-admin roster-import --org my-org --classroom short-name --db students.db --dry-run
course-c50-admin roster-import --org my-org --classroom short-name --csv classroom50_roster.csv

# Add one late student without a full CSV round trip
course-c50-admin roster-add --org my-org --classroom short-name --username alice \
  --first-name Alice --last-name Andersson --email alice@hus.edu.vn --section K70A9
...
- **`roster-import` writes the roster *and* sends organization invitations.** `gh teacher
  roster import` is an upsert, so rows already present are updated and rows absent from the
  CSV are left untouched — a partial CSV is safe, and rows that exist only on the remote
  roster are reported under `remoteOnly`, never removed. Once the commit lands the CLI
  invites every listed student who is not already a member, so the confirmation names both
  effects. The remote roster is read first, but a matching roster is not on its own proof
  that the work is done: because the commit lands before the invitations go out, a run that
  died in between leaves the roster matching and nobody invited. So when nothing would change
  the command reads the organization membership too, and short-circuits with `roster already
  matches the local database and every listed student is already in the organization; nothing
  to import` only when everyone is in. Otherwise it names the missing logins under
  `notYetMembers` and imports anyway, which is what sends the invitations. If the membership
  read fails — usually a token without `admin:org` — the command still exits 0, reports
  `membershipNotVerified`, and points at `--force`, which skips the check and imports a
  matching roster outright. Students with no GitHub username are listed under
  `skippedNoUsername` and do not change the exit code. The generated CSV lives in a private
  temp directory that is removed even when `gh` fails, unless `--keep-csv` is given; passing
  `--keep-csv` with `--csv` is refused, since that path creates no temporary file.
  `--dry-run` makes no `gh` call at all, so it reports a `plan` rather than a roster diff.
...
outcome is unverified, `4` a batch invite had failures. `roster-import` means `3` literally:
the roster is committed before invitations go out, so a failed call really does leave an
unknown state, and re-running is the recovery path because the import is idempotent. Student acceptance and student
...
| --- | --- | --- | --- |
| `python -m course_hoanganhduc.c50_agent` | Classroom50 | preflight, list-*, sync, export | assignment-add, assignment-remove, invite, roster-import, roster-add, download |
...
| `python -m course_hoanganhduc.gclass_agent` | Google Classroom | preflight, list-courses/students, sync | create-assignment, invite, unenroll, grade, download |
...
On a headless remote host, one terminal is enough:
...
With no browser to open, the command starts no loopback listener and prints the
consent URL instead. Open it on a machine that has a browser, grant access, and let
that browser fail to load `http://127.0.0.1:8911/` — the failure is expected and the
address bar holds the redirect. Paste that URL back at the hidden prompt in the same
terminal and authorization completes there. `--no-open-browser` takes the same path
on a host that does have a browser.

The paste is accepted only at the exact `http://127.0.0.1:8911/` boundary and with
one nonempty code and state; a redirect from an earlier attempt is refused by its
state, and the hidden prompt keeps the code out of shell history and process
arguments. Do not reuse an earlier redirect URL.

`complete-loopback --port PORT` still covers the other case: an authorization already
listening here whose browser cannot reach this host. Read the port from the redirect
URL and deliver it from a second terminal:
...
Verification (offline suite re-measured 2026-09-06; live smoke test recorded 2026-08-29):

- 259 offline regression tests passed across coursework construction/state handling,
  isolated authorization, the administrator CLIs, the Google Classroom invite path, the
  Classroom50 roster path, the CSV importer, restricted agent surfaces, and Classroom50
  integration; all 223 general CLI flags also passed parser dry-runs.
...
Invite local database students to a Google Classroom course (anyone already enrolled, already a
teacher, or already holding a pending invitation is skipped, so re-running is a no-op):

```bash
course --invite-google-classroom --google-course-id 1234567890
course --invite-google-classroom --gc-invite-domain hus.edu.vn --gc-invite-all
course --invite-google-classroom --gc-invite-section K70A9 --gc-invite-class "Intro to AI"
course --invite-google-classroom --gc-invite-email student1@hus.edu.vn,student2@hus.edu.vn
course --invite-google-classroom --gc-invite-role TEACHER --gc-invite-email ta@hus.edu.vn
course --invite-google-classroom --dry-run
```

Notes:
- Candidates come from the local database only. `--gc-invite-email` filters that database; an
  address that is not in it is reported as `skipped_not_in_db` instead of being invited, so import
  it first with `--add-google-sheet`.
- Without `--gc-invite-all` you pick from a numbered list.
- Successful invitations record `Google_ID`, `Google_Invitation_ID`, and
  `Google_Classroom_Invited_At` in the local database. `Google_ID` is filled only when empty and
  is never overwritten -- recording it is what makes the second run cost no profile lookups. The
  invitation id and the timestamp describe the latest invitation, so each successful invitation
  replaces them.
- A `403` from Classroom is a course-wide policy failure, not a per-student one, so the run stops
  there and the remaining candidates are reported as `skipped_aborted` rather than burning quota.
- A `400 FAILED_PRECONDITION` means the student already holds this role or greater, or the
  account is disabled. It is reported as `skipped_precondition` and does not count as a failure.
- After inviting, add the same students to Classroom50 with
  `course-c50-admin roster-import`; the command prints the filled-in next step for you.
...
course --add-google-sheet "<SHEET_URL>" --dry-run
```

Notes:
- Google Sheets API must be enabled for the project tied to your credentials.
- `--dry-run` reads and merges the sheet exactly as a real import would, prints every field
  a newer submission would replace, and writes nothing.
- A row whose student number is not 8 digits is kept, not dropped: the value is stored as
  `Invalid Student ID`, `Student ID` is left empty, and the student is named. A row that
  names nobody is dropped, also by name.

Import students from a roster CSV, with no Google account involved:

```bash
course --add-csv roster.csv
course --add-csv roster.csv --dry-run
```

The columns are fixed rather than auto-detected. `Student ID` and `Name` are required, and so is
at least one of `Email` and `GitHub Username` — without one of those the roster feeds neither
platform. `GitHub ID`, `Section`, and `Class` are read when present, anything else is ignored, and
a header that does not match is reported along with the columns actually read. Header names are
matched case- and accent-insensitively, so the Vietnamese spellings work: `Mã sinh viên`,
`Họ và tên`, `Email`, `GitHub username`, `Lớp`. Rows with no student id or no name are skipped and
counted rather than failing the run.

The file is decoded as UTF-8 (with or without a BOM, so Excel's "CSV UTF-8" export works), then
Windows-1252, then Latin-1, and the command says which encoding it used whenever it falls back —
the opposite trade-off from `course-c50-admin roster-import --csv`, which refuses anything but
UTF-8 because a mis-decoded name would be pushed to GitHub. `--dry-run` previews the first rows
and writes nothing.

This is not a replacement for `--add-file`, which still handles Excel, PDF, and the `MAT*.xlsx`
files with their own header quirks. `--add-csv` is for a clean roster CSV whose columns you
control.

Onboard a registration form to Google Classroom and Classroom50 in one pass:

```bash
course --onboard
course --onboard --onboard-csv registrations.csv --dry-run
course --onboard --google-course-id 1234567890 \
       --classroom50-org my-org --classroom50-classroom my-class
```

`--onboard` asks three questions — which Google Classroom course, which Classroom50 classroom,
which CSV — and either platform can be skipped. Skipping both is a useful run on its own: the
CSV is loaded into the database and the report tells you what the form contains, which is what
you want a week before term starts. Anything you pass on the command line is not asked again:
`--google-course-id`, `--classroom50-org`, `--classroom50-classroom`, `--onboard-csv`, and
`--classroom50-report` are the flags the other commands already use.

The CSV is expected to be a Google Form export, quirks included: bilingual headers like
`Họ và Tên (Full Name)`, the asterisk the form adds to a required question, both a school and a
personal address, students who submitted twice, and typos. Per student, the school address wins
and a missing or malformed one falls back to the personal one; the GitHub username is checked
against GitHub's account rule and then against `gh api /users/<name>`, which also yields the id
the Classroom50 roster wants. A username that could not be checked — no network, rate limit — is
reported as unverified rather than treated as wrong, so a bad connection cannot reject a class.

A row that cannot be used is skipped with its own reason and the run continues. Repeat
submissions collapse to the last one. Two students claiming the same GitHub username drop both,
by name, because the Classroom50 import would otherwise overwrite one with the other silently.

Nothing is written until you say so. The Classroom50 roster is pulled into the database first,
`gh teacher roster sync` is run in its reporting mode — exit 2 there means links are still
pending, and the run stops and asks you to `--write` them, because such a row would otherwise be
imported a second time for the same person — and both platforms are then read into one table:

```
Sinh viên      | Google Classroom    | Classroom50
---------------|---------------------|----------------------------
Nguyễn Văn A   | đã ghi danh         | có roster, đã vào org
Trần Thị B     | lời mời đang treo   | có roster, CHƯA vào org
Lê Văn C       | SẼ MỜI              | SẼ THÊM + MỜI
Phạm Thị D     | SẼ MỜI              | bỏ qua: GitHub không tồn tại
```

Only then does it ask to continue, and the Classroom50 import keeps its own confirmation, so a
confirmed run answers twice. `--dry-run` makes no `gh` call, no Google call, and no database
write; the table is therefore a real run's step, not a dry run's. Use
`--onboard-skip-github-check` to validate usernames by syntax alone when offline.
