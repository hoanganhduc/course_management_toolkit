...
def gh_teacher_available() -> Tuple[bool, str]:
    ...
    Presence only. This deliberately does not read `gh teacher --version`, so preflight
    cannot detect an extension older or newer than the one the argv builders were verified
    against; see the version-coupling note in `c50_cli_human`.
    """
    if not gh_on_path():
