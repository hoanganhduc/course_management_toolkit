...
from .utils import *
from .data import *
# ``import *`` skips underscore names, so the submission-source and state-token
# helpers have to be named to be in scope here.
from .data import (
    _normalize_c50_submission_state_token,
    _normalize_canvas_submission_state_token,
    _normalize_gc_submission_state_token,
    _normalize_submission_source,
)
from .canvas import *
from .canvas import *
...
def _print_classroom50_next_step(args, config, report):
    """Point at the Classroom50 half of onboarding after a real Google invite.

    The two halves stay separate commands (different confirmation gates and
    exit-code contracts), so the affordance is a filled-in next line rather
    than a chained call.
    """
    if not isinstance(report, dict) or report.get("dry_run"):
        return
    if not report.get("invited"):
        return
    try:
        from .c50_flags import _resolve_classroom, _resolve_org
    except Exception:
        return
    org = _resolve_org(args, config if isinstance(config, dict) else None) or "<ORG>"
    classroom = _resolve_classroom(args, config if isinstance(config, dict) else None) or "<SHORT>"
    print("Next, add the same students to Classroom50 (roster + organization):")
    print(f"  course-c50-admin roster-import --org {org} --classroom {classroom} --db <DB_PATH>")
...
def _prompt_submission_source():
    ...
    while True:
        ...
        if raw in ("canvas", "cv"):
            return "canvas"
        if raw in ("classroom50", "c50", "cl50"):
            return "classroom50"
...
def _prompt_submission_status(source):
    ...
    if not options:
        ...
        return None
    # ``options`` is empty for a source no lane recognises, so the lookup below
    # only ever runs on a source the normalizer named.
    ...
    while True:
        ...
        if source_key == "canvas":
            ...
            mapped = _normalize_c50_submission_state_token(raw)
        else:
...
def _collect_submission_assignment_titles(students, source):
    field_name = {
        "canvas": "Canvas Submissions",
        "classroom50": "Classroom50 Submissions",
    }.get(_normalize_submission_source(source), "Submissions")
    titles = set()
...
def _parse_submission_status_arg(raw):
    ...
    if ":" in value:
        ...
        if prefix in ("classroom50", "c50", "cl50"):
            return "classroom50", token, assignment
    return None, value, assignment


def _run_roster_audit(args, students):
    """Print whichever roster-audit report the flags asked for.

    The audit answers group-level questions -- who is missing, who typed what
    wrong, who looks like two people -- so it runs on its own rather than joining
    the per-student listing filters below it.
    """
    import shutil

    from . import roster_audit

    # A username that is spelled like a username but belongs to nobody costs the
    # student their whole repository, and nothing else in the audit can see it,
    # so the check runs unless it is turned off or ``gh`` is not installed.
    wanted = getattr(args, "audit_check_github", None)
    if wanted is None:
        wanted = shutil.which("gh") is not None
        if not wanted and args.verbose:
            print("[Audit] gh not found; skipping the GitHub account check")
    checks = None
    if wanted:
        checks = roster_audit.verify_github_accounts(students, verbose=args.verbose)
    report = roster_audit.audit_roster(
        students,
        checks=checks,
        school_domain=getattr(args, "audit_school_domain", None) or roster_audit.DEFAULT_SCHOOL_DOMAIN,
        expected_sections=getattr(args, "audit_section", None),
    )

    def emit(text, destination):
    ...
    if getattr(args, "list_missing_form", False):
        print(roster_audit.format_missing_form(report.missing_form))
    if getattr(args, "list_invalid_info", False):
        print(roster_audit.format_issues(report.issues))
    if getattr(args, "list_duplicate_accounts", False):
        print(roster_audit.format_duplicate_accounts(report.duplicate_accounts))
    if getattr(args, "audit_roster", None):
        emit(roster_audit.format_audit(report), args.audit_roster)
    if getattr(args, "classroom_announcement", None):
        course_code = args.course_code or get_cached_course_code() or ""
        emit(
            roster_audit.format_announcement(
                report,
                course_name=course_code,
                form_url=getattr(args, "announcement_form_url", None) or "",
                deadline=getattr(args, "announcement_deadline", None) or "",
                school_domain=getattr(args, "audit_school_domain", None)
                or roster_audit.DEFAULT_SCHOOL_DOMAIN,
            ),
            args.classroom_announcement,
        )
...
def main():
    ...
    db_group.add_argument('--list-missing-form', action='store_true',
                          help="List students on the roster who have not filled the registration form",
                          dest="list_missing_form")
    db_group.add_argument('--list-invalid-info', action='store_true',
                          help="List students whose registered information is wrong, naming the field and the fix",
                          dest="list_invalid_info")
    db_group.add_argument('--list-duplicate-accounts', action='store_true',
                          help="List students suspected of holding two Google Classroom accounts",
                          dest="list_duplicate_accounts")
    db_group.add_argument('--audit-roster', nargs='?', const=True,
                          help="Run every roster check at once (optional: output path)",
                          dest="audit_roster", metavar="TXT_FILE")
    db_group.add_argument('--classroom-announcement', nargs='?', const=True,
                          help="Draft the Google Classroom announcement asking students to fix their information (optional: output path)",
                          dest="classroom_announcement", metavar="TXT_FILE")
    db_group.add_argument('--audit-check-github', action='store_true', default=None,
                          help="Ask GitHub whether each registered username exists while auditing (default: on when gh is installed)",
                          dest="audit_check_github")
    db_group.add_argument('--no-audit-check-github', action='store_false',
                          help="Audit without asking GitHub whether each username exists",
                          dest="audit_check_github")
    db_group.add_argument('--audit-school-domain', type=str,
                          help="Email domain the school issues, for roster audits (default: hus.edu.vn)",
                          dest="audit_school_domain", metavar="DOMAIN")
    db_group.add_argument('--audit-section', action='append',
                          help="Course section that is valid for this course; repeatable",
                          dest="audit_section", metavar="NAME")
    db_group.add_argument('--announcement-form-url', type=str,
                          help="Registration form link to put in the announcement",
                          dest="announcement_form_url", metavar="URL")
    db_group.add_argument('--announcement-deadline', type=str,
                          help="Deadline to state in the announcement",
                          dest="announcement_deadline", metavar="TEXT")
    ...
    db_group.add_argument('--add-csv', type=str,
                          help="Import students from a roster CSV with fixed columns (Student ID, Name, and Email and/or GitHub Username)",
                          dest="add_csv", metavar="CSV_FILE")
    db_group.add_argument('--onboard', action='store_true',
                          help="Interactive onboarding: pick a Google Classroom course and/or a Classroom50 classroom, pick a registration CSV, validate it, and add whoever is missing",
                          dest="onboard")
    db_group.add_argument('--onboard-csv', type=str,
                          help="Registration CSV for --onboard (skips the file prompt)",
                          dest="onboard_csv", metavar="CSV_FILE")
    db_group.add_argument('--onboard-skip-github-check', action='store_true',
                          help="For --onboard: check GitHub usernames by syntax only, without calling the API (offline)",
                          dest="onboard_skip_github_check")
    db_group.add_argument('--no-open-browser', action='store_true',
                          help="For --onboard: never open a browser for Google authorization; print the URL to open elsewhere, then paste the failed redirect back at the prompt",
    ...
    gclass_group.add_argument('--gc-ocr-lang', type=str,
                              ...
                              dest="gc_ocr_lang")
    gclass_group.add_argument('--invite-google-classroom', '-igc', action='store_true',
                              help="Invite local database students to a Google Classroom course (skips anyone already enrolled or invited)",
                              dest="invite_google_classroom")
    gclass_group.add_argument('--gc-invite-email', type=str,
                              help="Email(s) from the local database to invite (comma-separated)",
                              dest="gc_invite_email")
    gclass_group.add_argument('--gc-invite-domain', type=str,
                              help="Email domain(s) to match for Google Classroom invite (comma-separated)",
                              dest="gc_invite_domain")
    gclass_group.add_argument('--gc-invite-section', type=str,
                              help="Section to match for Google Classroom invite",
                              dest="gc_invite_section")
    gclass_group.add_argument('--gc-invite-class', type=str,
                              help="Class to match for Google Classroom invite",
                              dest="gc_invite_class")
    gclass_group.add_argument('--gc-invite-role', type=str, choices=['STUDENT', 'TEACHER'], default='STUDENT',
                              help="Role to invite matched people as (default: STUDENT)",
                              dest="gc_invite_role")
    gclass_group.add_argument('--gc-invite-all', action='store_true',
                              help="Invite all matched students without selection",
                              dest="gc_invite_all")
    ...
    audit_flags = (
        getattr(args, "list_missing_form", False),
        getattr(args, "list_invalid_info", False),
        getattr(args, "list_duplicate_accounts", False),
        getattr(args, "audit_roster", None),
        getattr(args, "classroom_announcement", None),
    )
    if any(audit_flags):
        _run_roster_audit(args, students)
    ...
    if getattr(args, "onboard", False):
        from .onboard import resolve_presets, run_onboarding

        # Each of the three carries the tier it came from, so a value read from
        # config.json is offered as a default and confirmed rather than used
        # silently; a flag stays an instruction.
        ...
        run_onboarding(
            db_path=db_path,
            csv_path=getattr(args, "onboard_csv", None),
            google_course_id=presets.google_course_id,
            c50_org=presets.c50_org,
            c50_classroom=presets.c50_classroom,
            skip_github_check=bool(getattr(args, "onboard_skip_github_check", False)),
            dry_run=bool(args.dry_run),
            verbose=args.verbose,
            report_path=getattr(args, "classroom50_report", None),
            credentials_path=credentials_path,
            token_path=token_path,
            # None lets the flow open a browser only where there is one; the flag
            # forces the printed-URL path even on a machine that has one.
            open_browser=False if getattr(args, "no_open_browser", False) else None,
        )

    if getattr(args, "add_csv", None):
        from .roster_csv import RosterCsvError, import_students_from_csv

        for csv_file in _expand_cli_paths(args.add_csv):
            ...
            try:
                ...
                print(f"Failed to import students from {csv_file}: {exc}")
    ...
    if getattr(args, "invite_google_classroom", False):
        course_id = args.google_course_id if hasattr(args, "google_course_id") and args.google_course_id else None
        if not course_id:
            course_id = config.get("GOOGLE_CLASSROOM_COURSE_ID") if isinstance(config, dict) else None
        credentials_path, token_path = _resolve_google_paths(
        ...
        report = invite_students_to_google_classroom(
            course_id=course_id,
            emails=args.gc_invite_email,
            domains=args.gc_invite_domain,
            section=args.gc_invite_section,
            class_name=args.gc_invite_class,
            role=args.gc_invite_role,
            apply_all=bool(args.gc_invite_all),
            credentials_path=credentials_path,
            token_path=token_path,
            db_path=db_path,
        ...
        _print_classroom50_next_step(args, config, report)
        ...
        while True:
            ...
            if choice == '1':
                ...
                if args.google_credentials_path or args.google_token_path:
                    ...
                    credentials_path = synced["credentials_path"]
                    token_path = synced["token_path"]
                mode_raw = input("Filter (all/domain/email/section/class) [all]: ").strip().lower()
                mode = mode_raw or "all"
                domains = None
                emails = None
                section = None
                class_name = None
                if mode == "email":
                    ...
                    section = input("Section to match (or 'q' to quit): ").strip()
                    if section.lower() in ('q', 'quit'):
                        continue
                elif mode == "class":
                    class_name = input("Class to match (or 'q' to quit): ").strip()
                    if class_name.lower() in ('q', 'quit'):
                        continue
                role_raw = input("Invite as (student/teacher) [student]: ").strip().lower()
                role = "TEACHER" if role_raw in ("teacher", "t") else "STUDENT"
                apply_all_raw = input("Invite all matched students? (y/n) [n]: ").strip().lower()
                apply_all = apply_all_raw in ("y", "yes")
                dry_run_raw = input("Dry-run only (no invitations sent)? (y/n) [n]: ").strip().lower()
                dry_run = dry_run_raw in ("y", "yes")
                report = invite_students_to_google_classroom(
                    course_id=course_id,
                    emails=emails,
