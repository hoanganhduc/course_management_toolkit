...
from .roster_audit import DEFAULT_SCHOOL_DOMAIN as ROSTER_AUDIT_DEFAULT_DOMAIN
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    def _audit_args(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
        """The options every roster-audit subcommand shares."""
        p.add_argument("--db", default="students.db")
        p.add_argument(
            "--school-domain",
            default=ROSTER_AUDIT_DEFAULT_DOMAIN,
            help="Email domain the school issues (default: %(default)s)",
        )
        p.add_argument(
            "--section",
            action="append",
        ...
        p.add_argument(
            "--check-github",
            action="store_true",
            help="Ask GitHub whether each username exists (read-only network calls)",
        )
        p.add_argument("--text", action="store_true", help="Human-readable output")
        return p

    _audit_args(sub.add_parser("audit", help="Run every roster check at once"))
    _audit_args(
        sub.add_parser("list-missing-form", help="List students who have not filled the form")
    )
    _audit_args(
        sub.add_parser(
            "list-invalid-info", help="List students whose entered information is wrong"
        )
    )
    _audit_args(
        sub.add_parser(
            "list-duplicate-accounts",
            help="List students suspected of holding two Classroom accounts",
        )
    )
    p_ann = _audit_args(
        sub.add_parser("announcement", help="Draft the Google Classroom announcement")
    )
    p_ann.add_argument("--course-name", default="", help="Course name to put in the heading")
    p_ann.add_argument("--form-url", default="", help="Link to the registration form")
    p_ann.add_argument("--deadline", default="", help="Deadline to state in the announcement")
    ...
    try:
        ...
        if args.cmd in {
            ...
            checks = (
                roster_audit.verify_github_accounts(students)
            ...
            report = roster_audit.audit_roster(
                ...
                expected_sections=args.section,
            )
            ...
            if args.text:
                text = {
                    "audit": roster_audit.format_audit,
                    "list-missing-form": lambda r: roster_audit.format_missing_form(
                        r.missing_form
                    ),
                    "list-invalid-info": lambda r: roster_audit.format_issues(r.issues),
                    "list-duplicate-accounts": lambda r: (
                        roster_audit.format_duplicate_accounts(r.duplicate_accounts)
                    ),
                }[args.cmd]
                print(text(report))
                return 0
            payload = roster_audit.report_to_dict(report)
            if args.cmd == "list-missing-form":
                payload = payload["missing_form"]
            elif args.cmd == "list-invalid-info":
                payload = payload["issues"]
            elif args.cmd == "list-duplicate-accounts":
                payload = payload["duplicate_accounts"]
