...
def normalize_columns(df, verbose=False):
    ...
    mapping = {
        ...
        "Gender": "Gender",
        "Date of Birth": "Dob",
        # The submission time decides which of two answers by one student is the
        # current one, so it has to survive the import under a stable name
        # rather than only when the form happens to be in English.
        "Timestamp": "Timestamp",
...
def read_students_from_excel_csv(file_path, db_path=None, verbose=False, preview_only=False, preview_rows=5, sheet_name=None, sheet_selection='first', dry_run=False):
    """
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing rows"):
        ...
        sid_text = "" if sid is None else str(sid).strip()
        if sid_text.lower() in {"nan", "none"}:
            sid_text = ""
        sid_invalid = bool(sid_text) and not is_valid_student_id(sid_text)
        ...
        # Add default email if student has student id but no email.  A malformed
        # id would build an address that belongs to nobody, so it is skipped
        # here and reported below instead.
        if sid_text and not sid_invalid and (not email or str(email).strip() == ""):
            student_dict["Email"] = f"{sid_text}@hus.edu.vn"
        ...
        # A row with an unusable name is not a student and is dropped; a row
        # with a malformed student id still is one, so it is kept with the bad
        # value moved out of the way.  Both are named on stdout: the bare
        # `continue` that used to stand here is what made a student who had
        # filled the form indistinguishable from one who never did.
        row_email = str(student_dict.get("Email") or "").strip()
        if not row_email or row_email.lower() == "nan":
            row_email = "(no email)"
        if is_strange_name(name):
            dropped_rows.append((name or "(no name)", row_email))
            continue
        if sid_invalid:
            flagged_rows.append((name, row_email, sid_text))
            student_dict["Invalid Student ID"] = sid_text
            student_dict["Student ID"] = ""

        new_students.append(Student(**student_dict))

    if dropped_rows:
        print(f"Notice: dropped {len(dropped_rows)} row(s) that do not name a student:")
        for dropped_name, dropped_email in dropped_rows:
            print(f"  {dropped_name} <{dropped_email}>")
    if flagged_rows:
        print(
            f"Notice: kept {len(flagged_rows)} row(s) whose student id is not 8 digits. "
            "Student ID was left empty and the value stored as 'Invalid Student ID':"
        )
        for flagged_name, flagged_email, flagged_sid in flagged_rows:
            print(f"  {flagged_name} <{flagged_email}>: {flagged_sid!r}")
    ...
    # Merging is _dedup_students' job and is reached through save_database; no
    # second merge rule is invented here.  The copy that used to live in this
    # function only ever filled empty fields, so a student who submitted the
    # form again to correct an answer never reached the database.
    if db_path and not dry_run:
        save_database(all_students, db_path, verbose=verbose, audit_source="import-excel-csv")
        unique_students = load_database(db_path, verbose=verbose)
    else:
        # Same merge, in memory.  A dry run still reports what would change,
        # which is the whole point of running one before an import.
        merged = _dedup_students(refine_database(all_students, verbose=verbose))
        unique_students = merged.students
        if dry_run:
            if merged.updates:
                print(f"Dry run: {len(merged.updates)} field(s) would be updated from a newer form submission.")
                for update in merged.updates:
                    print(f"  {update.student}: {update.field}: {update.old!r} -> {update.new!r}")
            else:
                print("Dry run: no stored field would be updated.")
...
def read_students_from_pdf(pdf_path, db_path=None, lang="auto", service=DEFAULT_OCR_METHOD, verbose=False, dry_run=False):
    """
    ...
    # Same rule as every other import lane: hand the rows to save_database and
    # let _dedup_students merge them.  The copy that used to stand here read two
    # names, email_s and email_u, that were never defined in this function.
    if db_path and not dry_run:
        save_database(all_students, db_path, verbose=verbose, audit_source="import-pdf")
        unique_students = load_database(db_path, verbose=verbose)
    else:
        unique_students = _dedup_students(refine_database(all_students, verbose=verbose)).students
...
def import_students_from_file(
    ...
    ``dry_run`` reads and merges the file exactly as a real import would, prints
    what it found and what it would change, and writes nothing.
...
def refine_database(students, verbose=False):
    ...
    # Every removal is named.  This runs on each save, so a row lost here used
    # to look exactly like a row that was merged into another one.
    refined = []
    removed_rows = []
    for s in students:
        name = getattr(s, "Name", "")
        if is_strange_name(name):
            sid = str(getattr(s, "Student ID", "") or "").strip() or "--------"
            removed_rows.append(f"{sid}  {str(name).strip() or '(no name)'}")
            continue
        refined.append(s)
    if removed_rows:
        prefix = "[RefineDatabase] " if verbose else "Notice: "
        print(f"{prefix}Removed {len(removed_rows)} student(s) with strange names from database:")
...
class FieldUpdate(NamedTuple):
    """One stored value that a newer form submission replaced."""
...
class DedupResult(NamedTuple):
    """Merged records, together with every value a newer submission replaced."""

    students: List[Any]
    updates: List[FieldUpdate]


# Never overwritten by a newer submission.  These are written by the Google
# Classroom and Classroom50 lanes, and no form can produce them, so a difference
# here means the two records were filled from different sources rather than that
# one of them is out of date.
_MERGE_PROTECTED_PREFIXES = ("Google_", "Additional ", "Classroom50 ")

# Any value that looks like an address, wherever it sits in the record.  A
# Google Classroom row and a form row for one student often share nothing but
# this, so it is a merge key rather than only a display field.
_MERGE_EMAIL_RE = re.compile(r"^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$")

# Section-like fields whose conflicting values are kept side by side rather than
# resolved, for one student appearing in two registration lists.
...
def _incoming_is_newer(stored, incoming) -> bool:
    """Whether ``incoming`` is a later submission by the same student.

    True only when both records carry a Timestamp that parses and the incoming
    one is strictly later.  Every other case is False: a Timestamp missing on
    either side, one that does not parse, or two equal ones.  So a record no
    form produced can neither overwrite nor be overwritten, re-importing the
    same sheet changes nothing, and the result does not depend on the order the
    records arrive in.
    """
    stored_ts = _parse_timestamp(getattr(stored, "Timestamp", None))
    if stored_ts is None:
        return False
    incoming_ts = _parse_timestamp(getattr(incoming, "Timestamp", None))
    if incoming_ts is None:
        return False
    try:
        return incoming_ts > stored_ts
    except TypeError:
        # One side carried a time zone and the other did not; refuse to guess.
        return False


def _dedup_students(students_to_merge) -> "DedupResult":
    """Merge records that describe the same student.

    Two records are the same student when they share a Google_ID, or a
    Student ID, or an email address without contradicting each other's
    Student ID, or a name while neither carries a Student ID.  This is the
    only merge rule in the package: every import lane hands its rows to
    ``save_database`` rather than collapsing duplicates on its own.

    A value already stored is kept unless the incoming record is a
    later submission by the same student, in which case the later
    answer wins and is reported.  Records with no readable Timestamp
    keep the older fill-what-is-empty behaviour, so the Google
    Classroom and Classroom50 lanes are unaffected.
    """
    def get_student_id(student):
        return _normalize_student_id(getattr(student, "Student ID", None))

    def get_google_id(student):
        value = getattr(student, "Google_ID", None)
        return str(value).strip() if value else ""

    def get_email(student):
        for value in student.__dict__.values():
            text = str(value).strip()
            if text and _MERGE_EMAIL_RE.match(text):
                return text.lower()
        return ""
    ...
    for s in students_to_merge:
        ...
        name_s = get_name(s)
        gid_s = get_google_id(s)
        email_s = get_email(s)
        found_duplicate = False
        for u in unique_students:
            sid_u = get_student_id(u)
            name_u = get_name(u)
            gid_u = get_google_id(u)
            email_u = get_email(u)
            is_duplicate = False
            if gid_s and gid_u and gid_s == gid_u:
                # One Google account is one person, whatever the two rows say
                # their name or student id is.  This is what collapses the extra
                # row the Classroom lane creates for a student who joined with a
                # personal address and filled the form with the school one.
                is_duplicate = True
            elif sid_s and sid_u and sid_s == sid_u:
                is_duplicate = True
            elif email_s and email_u and email_s == email_u and not (sid_s and sid_u):
                # Two records that disagree about the student id are left alone;
                # a shared address is only evidence of one student when at least
                # one side has no id of its own to contradict it.
                is_duplicate = True
            elif name_s and name_u and name_s == name_u and not sid_s and not sid_u:
                is_duplicate = True
            if is_duplicate:
                newer = _incoming_is_newer(u, s)
                label = get_student_id(s) or get_name(s)
                overwritten = set()
                for k, v in s.__dict__.items():
                    if k not in u.__dict__ or not u.__dict__[k]:
                        ...
                        u.__dict__[k] = v
                    elif newer and v and u.__dict__[k] != v and not k.startswith(_MERGE_PROTECTED_PREFIXES):
                        updates.append(FieldUpdate(label, k, str(u.__dict__[k]), str(v)))
                        overwritten.add(k)
                        u.__dict__[k] = v
                    elif k in _MERGE_SECTION_FIELDS and u.__dict__[k] != v and v:
                        u.__dict__[f"Additional {k}"] = v
                if "GitHub Username" in overwritten and not str(s.__dict__.get("GitHub ID", "") or "").strip():
                    # The stored id belongs to the account named by the old
                    # username.  roster import re-resolves the id from the
                    # username and fails the line when the two disagree, so
                    # an empty id is safe where a stale one is not.
                    stale = u.__dict__.get("GitHub ID", "")
                    if stale:
                        updates.append(FieldUpdate(label, "GitHub ID", str(stale), ""))
                        u.__dict__["GitHub ID"] = ""
                parked = str(u.__dict__.get("Invalid Student ID", "") or "").strip()
                current = str(u.__dict__.get("Student ID", "") or "").strip()
                if parked and re.fullmatch(r"\d{8}", current):
                    # The import writes the two fields together: a number it
                    # cannot accept is parked here and the field left empty.  A
                    # later submission fills that empty field through the first
                    # branch above, but no branch clears, so the parked number
                    # would outlive the mistake and the audit would go on
                    # reading it as the student's current answer.
                    updates.append(FieldUpdate(label, "Invalid Student ID", parked, ""))
                    u.__dict__["Invalid Student ID"] = ""
                found_duplicate = True
                break
        if not found_duplicate:
            unique_students.append(s)
    return DedupResult(unique_students, updates)
...
def save_database(students, db_path, verbose=False, audit_source=None):
    ...
    dedup = _dedup_students(students_refined)
    students_refined = dedup.students
    ...
    if dedup.updates:
        print(f"Notice: {len(dedup.updates)} field(s) updated from a newer form submission.")
        for update in dedup.updates:
            print(f"  {update.student}: {update.field}: {update.old!r} -> {update.new!r}")
...
# Classroom50 collects on demand rather than continuously, so two of its states
# say what the last snapshot could not answer rather than what the student did.
# See course_hoanganhduc.c50_scores.submission_state for when each is written.
_C50_SUBMISSION_STATE_EXPLANATIONS = {
    "SUBMITTED": "submitted",
    "NOT_SUBMITTED": "not submitted",
    "NOT_COLLECTED_YET": "not collected yet (nothing was collected after the assignment opened)",
    "UNKNOWN_STALE": "unknown (the snapshot was collected before the deadline)",
}
_C50_SUBMISSION_STATE_ORDER = [
    "SUBMITTED",
    "NOT_SUBMITTED",
    "NOT_COLLECTED_YET",
    "UNKNOWN_STALE",
...
# Every spelling of a submission lane that a caller may pass, mapped to the one
# this module dispatches on.  A name absent from here is an error rather than a
# default: the branch below used to be a catch-all, so a mistyped source answered
# with Google Classroom data and said nothing about it.
_SUBMISSION_SOURCE_ALIASES = {
    "": "google",
    "none": "google",
    "google": "google",
...
def _normalize_submission_source(source):
    """Which submission lane ``source`` names, or ``None`` if it names none."""
    return _SUBMISSION_SOURCE_ALIASES.get(str(source).strip().lower())
...
def get_c50_submission_status_options():
    return [
        (state, _C50_SUBMISSION_STATE_EXPLANATIONS.get(state, ""))
        for state in _C50_SUBMISSION_STATE_ORDER
    ]

def get_submission_status_options(source):
    source_key = _normalize_submission_source(source)
    if source_key == "canvas":
        return get_canvas_submission_status_options()
    if source_key == "classroom50":
        return get_c50_submission_status_options()
...
def _normalize_c50_submission_state_token(token):
    if token is None:
        return None
    raw = str(token).strip()
    if not raw:
        return None
    # The stored value is "unknown (stale)"; the brackets are dropped before the
    # usual normalization so the written form and the typed form agree.
    cleaned = re.sub(r"[-\s]+", "_", re.sub(r"[()]", " ", raw).strip()).upper()
    alias_map = {
        "TURNED_IN": "SUBMITTED",
        "UNSUBMITTED": "NOT_SUBMITTED",
        "MISSING": "NOT_SUBMITTED",
        "NOT_COLLECTED": "NOT_COLLECTED_YET",
        "STALE": "UNKNOWN_STALE",
        "UNKNOWN": "UNKNOWN_STALE",
    }
    mapped = alias_map.get(cleaned, cleaned)
    if mapped in _C50_SUBMISSION_STATE_EXPLANATIONS:
        return mapped
    return None
...
def _normalize_c50_submission_states(raw):
    if raw is None:
        return []
    if isinstance(raw, (list, tuple, set)):
        tokens = [str(v).strip() for v in raw if str(v).strip()]
    ...
    seen = set()
    for token in tokens:
        mapped = _normalize_c50_submission_state_token(token)
        if mapped and mapped not in seen:
            normalized.append(mapped)
            seen.add(mapped)
    return normalized
...
def _c50_submission_state_explanation(state):
    if not isinstance(state, str) or not state.strip():
        return ""
    state_key = _normalize_c50_submission_state_token(state)
    return _C50_SUBMISSION_STATE_EXPLANATIONS.get(state_key or "", "")
...
def en_to_vn_field(field, verbose=False):
    ...
    field_vn_map = {
        ...
        "Classroom50 Scores Collected At": "Thời điểm thu điểm Classroom50",
        "Classroom50 Group": "Nhóm Classroom50",
        "Classroom50 Group Conflicts": "Xung đột nhóm Classroom50",
        # Final-project grade entered by hand
        "Override Final Project": "Điểm final project (nhập tay)",
        "Override Final Project Reason": "Ghi chú điểm final project",
        # Group lane
        "MiniProject_Group_Repo": "Repo nhóm mini-project",
        "MiniProject_Group_Members_GitHub": "Thành viên nhóm mini-project (GitHub)",
        "MiniProject_Group_Source": "Nguồn thông tin nhóm mini-project",
        "MiniProject_Group_Conflict": "Xung đột nhóm mini-project",
        # Issue lane
        "MiniProject_Issue_Number": "Số hiệu issue mini-project",
        "MiniProject_Issue_URL": "Link issue mini-project",
        "MiniProject_Issue_Status": "Nhãn trạng thái issue mini-project",
        "MiniProject_Issue_Title": "Tiêu đề issue mini-project",
        "MiniProject_Issue_Topic_Source": "Nguồn chủ đề mini-project",
        "MiniProject_Issue_Group_Name": "Tên nhóm khai trong issue mini-project",
        "MiniProject_Issue_Repo": "Repo khai trong issue mini-project",
        "MiniProject_Issue_Proposal_Commit": "Commit proposal mini-project",
        "MiniProject_Issue_Members": "Thành viên khai trong issue mini-project",
        "MiniProject_Issue_Final_Submitted": "Đã nộp bài cuối mini-project",
        "MiniProject_Issue_Content_Hash": "Mã băm nội dung issue mini-project",
...
_C50_CONTAINER_FIELDS = (
    ...
    "Classroom50 Group",
    "Classroom50 Group Conflicts",
)


def _classroom50_detail_lines(data):
    """Bilingual lines for the Classroom50 containers, for both detail views.
    ...
    """
    lines = []
    overrides = data.get("Classroom50 Score Overrides")
    overrides = overrides if isinstance(overrides, dict) else {}

    grades = data.get("Classroom50 Grades")
    if isinstance(grades, dict) and grades:
        total = 0.0
        total_max = 0.0
        lines.append("Classroom50 grades (Điểm Classroom50):")
        for slug in sorted(grades.keys()):
            info = grades.get(slug) or {}
            suffix = ""
            if isinstance(info, dict):
                ...
                if info.get("group"):
                    marks.append("nhóm")
                if info.get("late"):
                    marks.append("nộp muộn")
                if overrides.get(slug):
                    marks.append("giảng viên đã sửa")
                if info.get("datetime"):
                    marks.append(str(info.get("datetime")))
                suffix = f" ({', '.join(marks)})" if marks else ""
            else:
                value = str(info)
            lines.append(f"  - {slug}: {value}{suffix}")
        if total_max > 0:
            lines.append(
                f"Total score (Classroom50) (Tổng điểm Classroom50): {total}/{total_max}"
            )
        elif total > 0:
    ...
    orphans = sorted(
        slug for slug in overrides
        if overrides.get(slug) and not (isinstance(grades, dict) and slug in grades)
    )
    if orphans:
        lines.append(
            "Classroom50 score overrides (Điểm Classroom50 giảng viên đã sửa):"
        ...
        for slug in orphans:
            lines.append(f"  - {slug}: giảng viên đã sửa ở thượng nguồn")

    submissions = data.get("Classroom50 Submissions")
    if isinstance(submissions, dict) and submissions:
        lines.append("Classroom50 submissions (Trạng thái nộp bài Classroom50):")
        for slug in sorted(submissions.keys()):
            state = submissions.get(slug)
            explanation = _c50_submission_state_explanation(state)
            suffix = f" ({explanation})" if explanation else ""
            lines.append(f"  - {slug}: {state}{suffix}")

    details = data.get("Classroom50 Submission Details")
    if isinstance(details, dict) and details:
        lines.append("Classroom50 submission details (Chi tiết bài nộp Classroom50):")
        for slug in sorted(details.keys()):
            payloads = details.get(slug)
            if not isinstance(payloads, list):
                lines.append(f"  - {slug}: {payloads}")
                continue
            lines.append(f"  - {slug}: {len(payloads)} bản ghi")
            for payload in payloads:
                if not isinstance(payload, dict):
                    lines.append(f"      * {payload}")
                    continue
                when = payload.get("datetime") or "unknown"
                ...
                lines.append(f"      * {when}: {value} {commit}".rstrip())
                tests = payload.get("tests")
                if isinstance(tests, list) and tests:
                    passed = sum(
                        1 for t in tests if isinstance(t, dict) and t.get("passed")
                    )
                    lines.append(f"          tests: {passed}/{len(tests)} passed")

    groups = data.get("Classroom50 Group")
    if isinstance(groups, dict) and groups:
        lines.append("Classroom50 group (Nhóm Classroom50):")
        for slug in sorted(groups.keys()):
            info = groups.get(slug) or {}
            if not isinstance(info, dict):
                lines.append(f"  - {slug}: {info}")
                continue
            head = " — ".join(
                str(part) for part in (info.get("group_name"), info.get("repo")) if part
            )
            source = info.get("source")
            tail = f" [{source}]" if source else ""
            lines.append(f"  - {slug}: {head or slug}{tail}")
            if info.get("founder"):
                lines.append(f"      Founder (Nhóm trưởng): {info.get('founder')}")
            members = info.get("members")
            if isinstance(members, (list, tuple)) and members:
                joined = ", ".join(str(member) for member in members)
                lines.append(f"      Members (Thành viên): {joined}")
            elif members:
                lines.append(f"      Members (Thành viên): {members}")

    conflicts = data.get("Classroom50 Group Conflicts")
    if isinstance(conflicts, dict) and conflicts:
        lines.append("Classroom50 group conflicts (Xung đột nhóm Classroom50):")
        for slug in sorted(conflicts.keys()):
            reasons = conflicts.get(slug)
            if isinstance(reasons, (list, tuple)):
                for reason in reasons:
                    lines.append(f"  - {slug}: {reason}")
            else:
                lines.append(f"  - {slug}: {reasons}")
...
def print_all_student_details(students, db_path=None, verbose=False, sort_method=None):
    ...
    for idx, s in enumerate(sorted_students, 1):
        ...
        remaining_keys = [
            ...
            and k not in _C50_CONTAINER_FIELDS
        ...
        for line in _classroom50_detail_lines(data):
            print(line)
        print('-' * 40)
...
def export_to_excel(students, file_path=None, db_path=None, verbose=False):
    ...
    for s in students:
        ...
        c50_subs = getattr(s, "Classroom50 Submissions", None)
        if isinstance(c50_subs, dict):
            for slug in c50_subs.keys():
                if isinstance(slug, str) and slug.strip():
                    c50_slugs.add(slug.strip())
    gc_titles = sorted(gc_titles, key=lambda t: t.lower())
    canvas_titles = sorted(canvas_titles, key=lambda t: t.lower())
    c50_slugs = sorted(c50_slugs, key=lambda t: t.lower())
    ...
    export_fields = [
        ...
        # Hand-entered final-project grade and its free-text reason.  The grade
        # may arrive as a number or as a string, so it is passed through as
        # stored rather than coerced.
        ("Override Final Project", lambda s: getattr(s, "Override Final Project", "")),
        ("Override Final Project Reason", lambda s: getattr(s, "Override Final Project Reason", "")),
        ("Classroom50 Scores Collected At", lambda s: getattr(s, "Classroom50 Scores Collected At", "")),
        ("MiniProject_Group_Repo", lambda s: getattr(s, "MiniProject_Group_Repo", "")),
        ("MiniProject_Group_Members_GitHub", lambda s: getattr(s, "MiniProject_Group_Members_GitHub", "")),
        ("MiniProject_Group_Source", lambda s: getattr(s, "MiniProject_Group_Source", "")),
        ("MiniProject_Group_Conflict", lambda s: getattr(s, "MiniProject_Group_Conflict", "")),
        ("MiniProject_Issue_Number", lambda s: getattr(s, "MiniProject_Issue_Number", "")),
        ("MiniProject_Issue_URL", lambda s: getattr(s, "MiniProject_Issue_URL", "")),
        ("MiniProject_Issue_Status", lambda s: getattr(s, "MiniProject_Issue_Status", "")),
        ("MiniProject_Issue_Title", lambda s: getattr(s, "MiniProject_Issue_Title", "")),
        ("MiniProject_Issue_Topic_Source", lambda s: getattr(s, "MiniProject_Issue_Topic_Source", "")),
        ("MiniProject_Issue_Group_Name", lambda s: getattr(s, "MiniProject_Issue_Group_Name", "")),
        ("MiniProject_Issue_Repo", lambda s: getattr(s, "MiniProject_Issue_Repo", "")),
        ("MiniProject_Issue_Proposal_Commit", lambda s: getattr(s, "MiniProject_Issue_Proposal_Commit", "")),
        ("MiniProject_Issue_Members", lambda s: getattr(s, "MiniProject_Issue_Members", "")),
        ("MiniProject_Issue_Final_Submitted", lambda s: getattr(s, "MiniProject_Issue_Final_Submitted", "")),
        ("MiniProject_Issue_Content_Hash", lambda s: getattr(s, "MiniProject_Issue_Content_Hash", "")),
    ]
    ...
    c50_headers = [f"C50: {slug}" for slug in c50_slugs]
    
    all_headers = base_headers + gc_headers + canvas_headers + c50_headers
    ...
    for header in c50_headers:
        # header is "C50: {slug}"
        slug = header.replace("C50: ", "")
        header_map[header] = {"type": "c50", "title": slug}
    ...
    for s in tqdm(students, desc="Exporting"):
        ...
        c50_subs = getattr(s, "Classroom50 Submissions", None) if isinstance(getattr(s, "Classroom50 Submissions", None), dict) else {}
        ...
        for header in selected_headers:
            ...
            if info["type"] == "field":
                ...
                row[header] = c50_subs.get(info["title"], "")
...
def list_students_by_submission_status(students, status, source="google", assignment=None, db_path=None, verbose=False):
    ...
    if source_key == "canvas":
    ...
    elif source_key == "classroom50":
        statuses = _normalize_c50_submission_states(status)
        field_name = "Classroom50 Submissions"
        explain = _c50_submission_state_explanation
        normalize = _normalize_c50_submission_state_token
        details_field = "Classroom50 Submission Details"
        ...
        print(
            f"Unknown submission source '{source}'. "
            f"Use one of: google, canvas, classroom50."
        )
        return []
    if not statuses:
