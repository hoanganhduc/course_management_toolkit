...
MAX_PASTED_REDIRECT_URL_CHARS = 8192
...
# Nothing listens on this port during a pasted authorization. The browser is meant
# to fail to reach it so that the operator can copy the failed URL out of the
# address bar; Google accepts any loopback port for an installed-app client, and a
# port of its own keeps a redirect from the legacy lane from being pasted in here.
COURSEWORK_PASTE_REDIRECT_PORT = 8911
COURSEWORK_PASTE_REDIRECT_URI = f"http://127.0.0.1:{COURSEWORK_PASTE_REDIRECT_PORT}/"
_PASTE_ATTEMPTS = 3


def _browser_available() -> bool:
    """Whether this host has a browser ``run_local_server`` could open.
    ...
    which is the normal state on a headless host. Asking first turns a crash that
    hides the URL into a printed URL the operator can open elsewhere.
    """
...
def _pasted_authorization_response(raw: Any) -> Tuple[Optional[str], Optional[str]]:
    """Validate a pasted browser redirect into the response ``fetch_token`` reads.

    Returns ``(url, None)`` when the paste is usable, or ``(None, reason)``. The
    accepted boundary is the one ``complete-loopback`` enforces: a single loopback
    host and port, no user info, no control characters, and exactly one nonempty
    code and state. The scheme is rewritten to https because oauthlib refuses to
    read an OAuth response over http; only the query is read from that string, and
    the code is still exchanged against Google's own token endpoint.
    """
    ...
    if not url:
        return None, "nothing was pasted"
    if len(url) > MAX_PASTED_REDIRECT_URL_CHARS:
    ...
    if (
        ...
        or parsed_port != COURSEWORK_PASTE_REDIRECT_PORT
        or parsed.username is not None
        or parsed.password is not None
        or parsed.path != "/"
        or parsed.fragment
    ):
        return None, f"that is not the {COURSEWORK_PASTE_REDIRECT_URI} redirect"
    values: Dict[str, List[str]] = {}
    for key, value in query_pairs:
        values.setdefault(key, []).append(value)
    if "error" in values:
        return None, "Google returned an OAuth error instead of an authorization code"
    if (
        ...
        return None, "the pasted URL must carry one nonempty state and code"
    return urlunsplit(("https", parsed.netloc, parsed.path, parsed.query, "")), None


def _authorize_by_pasted_redirect(
    flow: Any,
    *,
    secret_input_fn: Optional[Callable[[str], str]] = None,
    output_fn: Optional[Callable[[str], None]] = None,
) -> Any:
    """Complete consent in this one terminal, on a host with no browser.

    ``run_local_server`` blocks inside ``handle_request()`` until a browser reaches
    its loopback port, so the process that prints the authorization URL cannot also
    ask for the redirect, and finishing it needed ``complete-loopback`` in a second
    terminal. Setting the redirect URI here and exchanging a pasted response instead
    keeps the whole authorization in the terminal the operator already has open. The
    authorization parameters match the browser call so both paths request the same
    grant, and the paste stays hidden because it carries a live authorization code.
    """

    read_secret = getpass.getpass if secret_input_fn is None else secret_input_fn
    write_line = print if output_fn is None else output_fn

    flow.redirect_uri = COURSEWORK_PASTE_REDIRECT_URI
    authorization_url, _state = flow.authorization_url(
        access_type="offline",
        prompt="consent",
    )
    write_line("No browser on this host. Finish authorization like this:")
    write_line("  1. Open the URL below on a machine that has a browser.")
    write_line("  2. Sign in as the --account address and grant access.")
    write_line(f"  3. The browser then fails to load {COURSEWORK_PASTE_REDIRECT_URI}...")
    write_line("     That failure is expected; the URL is what matters.")
    write_line("  4. Copy that failed URL from the address bar and paste it here.")
    write_line("")
    write_line(authorization_url)
    write_line("")

    for remaining in range(_PASTE_ATTEMPTS - 1, -1, -1):
        try:
            pasted = read_secret("Paste the browser redirect URL (input hidden): ")
        except (EOFError, KeyboardInterrupt, StopIteration):
            raise CredentialSecurityError(
                "Authorization was cancelled before a redirect URL was pasted"
            ) from None
        response, reason = _pasted_authorization_response(pasted)
        if response is not None:
            try:
                ...
                # A rejected exchange is not a typo the operator can fix by pasting
                # again: the code is single-use and short-lived, so the way back is
                # a fresh authorization URL.
                raise CredentialSecurityError(
                    "Google rejected the pasted redirect; authorize again"
                ) from exc
            return flow.credentials
        if remaining:
            write_line(f"Cannot use that paste: {reason}. Try again.")
    raise CredentialSecurityError(
        "No usable redirect URL was pasted; no token was stored"
    )
    ...
    *,
    ...
    with TokenFileLock(paths.token_path):
        ...
        if credentials is None:
            ...
            # ``None`` opens a browser only where one exists, so a headless host
            # takes the pasted-redirect path on its own instead of failing inside
            # ``webbrowser.get`` with the authorization URL still unprinted.
            ...
            try:
                ...
                with _suppress_oauth_secret_logging():
                    if use_browser:
                        ...
                        # No listener runs here, so ``timeout_seconds`` has nothing
                        # to bound; the operator is at the prompt instead.
                        credentials = _authorize_by_pasted_redirect(flow)
