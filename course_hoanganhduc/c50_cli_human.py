...
The argv builders here mirror rules enforced by `gh teacher` itself, so this module is
coupled to the installed extension version. Everything below was verified against v1.45.0
(commit a5a51293b65d), upgraded from v1.25.1 on 2026-09-05 and pinned there. The constants
that diverge from the installed CLI are marked where they are defined; the full list is in
the README under "Version coupling with `gh teacher`".
"""
...
# Slug shape enforced by `gh teacher assignment add` v1.25.1. The installed v1.45.0 raised
# its own cap to `{1,99}`, so this stays deliberately stricter: the CLI now also computes a
# per-repo name budget (student repos are `<classroom>-<slug>-<username>`) that this regex
# does not model, and rejecting early beats a slug the CLI accepts here and refuses later.
...
class HumanCLI:
    ...
    def assignment_add_argv(
        ...
        *,
        ...
        # v1.45.0 rejects two more, --submission-mode and --submission-tag; this builder
        # cannot emit either, so the list stays complete for everything it can produce.
        conflicting = [
            flag
            for flag, given in (
        ...
        if feedback_pr is not None:
            # Default is true in v1.25.1 and still in v1.45.0; disabling needs `=false`.
    ...
    def roster_list_argv(self, org: str, classroom: str) -> List[str]:
        # Deliberately duplicates AgentCLI.list_roster: AgentCLI is a closed method
        # map for the agent surface, and a human write path must not route through it.
        return [
            self._gh,
            "teacher",
            "roster",
    ...
    def roster_import_argv(self, org: str, classroom: str, csv_path: str) -> List[str]:
        return [
            self._gh,
            "teacher",
            "roster",
            ...
            _operand(org, "org"),
            _operand(classroom, "classroom"),
            _operand(csv_path, "csv path"),
        ]

    def roster_sync_argv(self, org: str, classroom: str) -> List[str]:
        # Report only, always: --write is deliberately not emitted, so no caller of
        # this builder can apply the sync. Applying it is the operator's own step.
        return [
            self._gh,
            "teacher",
            "roster",
    ...
    def roster_add_argv(
        ...
        *,
        ...
        argv = [self._gh, "teacher", "roster", "add"]
        for flag, value in (
            ...
            if value is None or not str(value).strip():
                continue
            argv += [flag, _operand(value, flag.lstrip("-"))]
        argv += [org, classroom, username]
        return argv
    ...
    def roster_list(self, org: str, classroom: str) -> Any:
        return self._json(self.roster_list_argv(org, classroom), op="roster_list")

    def group_list(self, org: str, classroom: str, slugs: Sequence[str]) -> Any:
        """The group repositories of the given assignments, and who is on each.

        There is no argv builder for this one because there is no ``gh teacher``
        command behind it.  Both group assignments in these classrooms are
        ``mode=group``, a shared repository reached through collaborators rather
        than a GitHub Team, and the upstream CLI refuses its team subcommand on
        such an assignment.  The repository list and the collaborator list are
        ordinary read-only ``gh api`` calls, so this method delegates to
        :mod:`c50_groups` and passes its own runner through.
        """
        from .c50_groups import list_group_repos, list_repo_collaborators

        out: List[Dict[str, Any]] = []
        for entry in list_group_repos(
            _operand(org, "org"),
            _operand(classroom, "classroom"),
            slugs,
            runner=self._runner,
        ):
            members = list_repo_collaborators(org, entry.repo, runner=self._runner)
            out.append(
                {
                    "slug": entry.slug,
                    "repo": entry.repo,
                    "founder": entry.founder,
                    "collaborators": [m.login for m in members],
                }
            )
        return out

    def roster_import(self, org: str, classroom: str, csv_path: str) -> RunResult:
        argv = self.roster_import_argv(org, classroom, csv_path)
        return self._checked(argv, code="roster_import_failed")

    def roster_sync(self, org: str, classroom: str) -> RunResult:
        """Run the report-only roster sync and return its result unchecked.

        Not routed through ``_checked``: this command follows
        ``terraform plan -detailed-exitcode``, where 2 means "changes pending",
        not "failed". Callers read ``returncode`` themselves -- 0 nothing to do,
        1 error or degraded read, 2 pending links.
        """
        return self._runner(self.roster_sync_argv(org, classroom))

    def roster_add(self, org: str, classroom: str, username: str, **kwargs) -> RunResult:
        argv = self.roster_add_argv(org, classroom, username, **kwargs)
        return self._checked(argv, code="roster_add_failed")
