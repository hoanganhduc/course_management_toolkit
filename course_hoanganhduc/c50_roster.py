...
# Header accepted by `gh teacher roster import` (ParseImportCSV). These are the canonical
# roster columns minus the trailing `role`, which import does not preserve. Verified on the
# pinned v1.45.0 and on the v1.25.1 this was written against; pinned by a test. The CSV is
# written without a BOM on purpose: the CLI's header match is exact.
CANONICAL_COLUMNS = [
    "username",
    "first_name",
    "last_name",
...
def parse_roster_payload(data: Any) -> List[Dict[str, Any]]:
    ...
    # roster list --json also carries a 7th field, role, on both v1.25.1 and the pinned
    # v1.45.0. It is dropped on purpose:
    # this toolkit never writes role, and gh teacher roster import does not preserve
    # trailing columns the way roster add/update does. Re-emitting a remote row
    # through this dialect would therefore erase its role.
    return out
...
def _export_username(student: Any) -> str:
    """Resolve the username export_roster_csv would write, or "" if there is none."""
    username = local_github_username(student) or ""
    if not username:
        # try raw GitHub Username with original case
        raw = getattr(student, "GitHub Username", None) or getattr(student, "GitHub", None)
        username = str(raw).lstrip("@").strip() if raw else ""
    return username


def partition_roster_candidates(students: Sequence[Any]) -> Tuple[List[Any], List[Any]]:
    """Split students into (exportable, missing_username), input order preserved.

    export_roster_csv drops a student with no GitHub username silently, which is
    acceptable for an export but not for a write: the operator has to see who is
    being left off the roster before confirming. This function answers that
    question without changing export_roster_csv itself.
    """
    exportable: List[Any] = []
    missing: List[Any] = []
    for student in students:
        (exportable if _export_username(student) else missing).append(student)
