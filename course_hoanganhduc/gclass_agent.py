...
# Mutating verbs live on the human CLI. They are named once so a verb added to the
# parser and a verb refused at dispatch can never drift apart.
REFUSED_VERBS = (
    "create",
    "create-assignment",
    "create-coursework",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for bad in REFUSED_VERBS:
        sub.add_parser(bad, help="(refused) not agent-safe")

    args = parser.parse_args(argv)

    try:
        if args.cmd in REFUSED_VERBS:
            refuse(args.cmd)
